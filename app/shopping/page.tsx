...
export default function ShoppingList() {
  ...
  async function loadItems(userId: string) {
    const { data } = await supabase.from('shopping_list').select('*').eq('user_id', userId).order('checked', { ascending: true }).order('created_at', { ascending: true })
    setItems(data || [])
  }
  ...
  return (
    <main className="min-h-screen bg-stone-50">
      <nav className="bg-white border-b border-stone-200 px-4 py-4 flex items-center gap-3">
        ...
        {checked.length > 0 && <button onClick={clearChecked} className="text-xs text-red-400 hover:text-red-600 font-medium">Slet afkrydsede</button>}
      </nav>

      <div className="max-w-lg mx-auto px-4 py-6 space-y-4">
        <div className="flex gap-2">
          <input value={newItem} onChange={e => setNewItem(e.target.value)} onKeyDown={e => e.key === 'Enter' && addItem()} placeholder="Tilføj vare..." className="flex-1 border border-stone-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-green-800 text-stone-800 bg-white" />
          <button onClick={addItem} disabled={adding || !newItem.trim()} className="bg-green-900 text-white rounded-xl px-4 py-3 text-sm font-medium hover:bg-green-800 disabled:opacity-50">Tilføj</button>
        </div>

        <button onClick={importFromMealPlan} disabled={importing} className="w-full border border-green-900 text-green-900 rounded-xl py-3 text-sm font-medium hover:bg-green-50 disabled:opacity-50">
          {importing ? 'Claude slår ingredienser sammen...' : 'Hent ingredienser fra ugens madplan'}
        </button>
          ...
          <div className="bg-white rounded-2xl border border-stone-200 divide-y divide-stone-100">
            {unchecked.map(item => (
              <div key={item.id} className="flex items-center gap-3 px-4 py-3">
                <button onClick={() => toggleItem(item.id, item.checked)} className="w-6 h-6 rounded-full border-2 border-stone-300 flex items-center justify-center flex-shrink-0 hover:border-green-700 transition-colors" />
        ...
        {checked.length > 0 && (
          <div>
            ...
            <div className="bg-white rounded-2xl border border-stone-100 divide-y divide-stone-50">
              {checked.map(item => (
                <div key={item.id} className="flex items-center gap-3 px-4 py-3">
                  <button onClick={() => toggleItem(item.id, item.checked)} className="w-6 h-6 rounded-full bg-green-900 border-2 border-green-900 flex items-center justify-center flex-shrink-0">
